...
namespace numl.Tests.SupervisedTests
{
  ...
  public class NeuralNetworkStructuralTests
  {
    ...
    [Theory]
    [InlineData(true, 2, 10, 8, new[]{ 4, 3, 1})]
    [InlineData(true, 3, 8, 8, new[]{ 4, 4, 4, 4})]
    [InlineData(false, 2, 10, 8, new[]{ 4, 3, 1})]
    [InlineData(false, 3, 8, 8, new[]{ 4, 4, 4, 4})]
    public void Network_Prune_Test(bool backwards, int pruneLayers, int input, int output, int[] hidden)
    {
      ...
      var newInput = backwards ? layers[0] + 1 : layers[pruneLayers] + 1;
      var newOutput = backwards ? layers[layers.Count - (pruneLayers + 1)] : layers[layers.Count - 1];
      var newHidden = layers.Skip(backwards ? 1 : pruneLayers + 1)
                            .Take(layers.Count - (pruneLayers + 2)).ToArray();

      var layers2 = new List<int>();
      layers2.Add(newInput);

      for (var l = 0; l < newHidden.Length; l++)
    ...
    [Theory]
    [InlineData(8, new[]{ 5}, 4, 3, new[]{ 2}, 1, true, true)]
    [InlineData(90, new[]{ 10, 8, 6}, 5, 7, new[]{ 4, 2, 3}, 10, true, true)]
    [InlineData(70, new[]{ 10, 8, 6}, 5, 7, new[]{ 4, 2, 3}, 10, true, false)]
    [InlineData(50, new[]{ 10, 8, 6}, 5, 7, new[]{ 4, 2, 3}, 10, false, true)]
    [InlineData(30, new[]{ 10, 8, 6}, 5, 7, new[]{ 4, 2, 3}, 10, false, false)]
    public void Network_Stack_1_Test(
      int input1,
      int[] hidden1,
      int output1,
      int input2,
      ...
      bool pruneOutputs)
    {
      ...
      layers.Add(input1 + 1);
      layers.AddRange(hidden1.Select(s => s + 1));
      if (!pruneOutputs)
        layers.Add(output1 + 1);

      if (!pruneInputs)
        layers.Add(input2 + 1);
      layers.AddRange(hidden2.Select(s => s + 1));
      layers.Add(output2);
    ...
    private static int VerifyNetwork(Network network)
    {
      ...
      for (var layer = 0; layer < network.Layers; layer++)
      {
        ...
        foreach (var node in nodes)
        {
          ...
          if (layer == 0 && node.In.Count > 0)
            errors++;
          if (layer == network.Layers - 1 && node.Out.Count > 0)
            errors++;
        }
      }
      ...
      if (network.Out.Any(a => double.IsNaN(a.Output) || double.IsInfinity(a.Output)))
        errors++;
