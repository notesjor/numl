...
namespace numl.Tests.SupervisedTests
{
  ...
  public class MultiClassLearnerTests
  {
    ...
    {
      var digits = Digit.GetTrainingDigits();

      var descriptor = Model.Descriptor.Create<Digit>();

      var model = MultiClassLearner.Learn(
        new Supervised.Regression.LogisticRegressionGenerator
        {
      ...
      var accuracy = count / tests.Count();
    ...
    public void Test_Digits_MultiClass_Classification_Autoencoded()
    {
      ...
      Func<IEnumerable<Digit>, IEnumerable<Digit>> encoding = s =>
        s.Select(
          d =>
            new Digit
            {
