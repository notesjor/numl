...
namespace numl.Tests.SupervisedTests
{
  ...
  public class LinearRegressionTests : BaseSupervised
  {
    ...
    public void Linear_Regression_Test_CostFunction()
    {
      ...
      Assert.Equal(303.95d, Math.Round(cost, 2));

      Assert.Equal(new[] {-15.3, 598.2}, grad.Select(s => Math.Round(s, 1)).ToArray());
    }
    ...
    public void Linear_Regression_Test_CostFunction_Regularized()
    {
      ...
      Assert.Equal(303.99, Math.Round(cost, 2));

      Assert.Equal(new[] {-15.3, 598.3}, grad.Select(s => Math.Round(s, 1)).ToArray());
    }
    ...
    public void Linear_Regression_Test_House_Predictions_Normal()
    {
      ...
      var actualEqns = 295107.0d;

      Almost.Equal(actualEqns, Math.Round(priceEqns, 0), 5000);
    }
    ...
    public void Linear_Regression_Test_House_Predictions_Regularized()
    {
      ...
      var actualGrad = 296500.0d;

      Almost.Equal(actualGrad, Math.Round(priceGrad, 0), 5000);
    }
  }
}