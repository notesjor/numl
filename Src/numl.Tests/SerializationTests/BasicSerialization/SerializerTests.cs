...
namespace numl.Tests.SerializationTests.BasicSerialization
{
  ...
  public class SerializerTests
  {
    ...
    public void ArrayTest()
    {
      ...
      test(
        $"[{Math.PI.ToString("r", CultureInfo.InvariantCulture)},{(-1 * Math.PI).ToString("r", CultureInfo.InvariantCulture)}]",
        new object[] {Math.PI, -1 * Math.PI});
    ...
    {
      var a = JsonReaderFromString(Math.PI.ToString("r", CultureInfo.InvariantCulture)).Read();
      Assert.Equal(Math.PI, a);
      var b = JsonReaderFromString((-1 * Math.PI).ToString("r", CultureInfo.InvariantCulture)).Read();
      Assert.Equal(-1 * Math.PI, b);
