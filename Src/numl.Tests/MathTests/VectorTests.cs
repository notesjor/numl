...
namespace numl.Tests.MathTests
{
  ...
  public class VectorTests
  {
    ...
    {
      var path = Path.Combine(BaseSerialization.GetPath(GetType()), "vector_serialize_test.json");

      // want to test "ugly" members in the vector
      Vector v1 = new[] {Math.PI, Math.PI / 2.3, Math.PI * 1.2, Math.PI, Math.PI / 2.3, Math.PI * 1.2};
