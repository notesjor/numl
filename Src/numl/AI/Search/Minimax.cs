...
namespace numl.AI.Search
{
  ...
  ///   Class Minimax.
  /// </summary>
  public class Minimax<TState, TSuccessor> : AdversarialSearch<TState, TSuccessor>
    ...
    where TSuccessor : class, ISuccessor
  {
    ...
    private Node Find(Node node, double initial, Func<double, double, double> limit, Func<Node, Node> f)
    {
      ...
      {
        if (!ProcessEvent(node, (TSuccessor) successor))
          return node;
    ...
    /// <param name="node">The node.</param>
    /// <returns>Node.</returns>
    public virtual Node Max(Node node) { return Find(node, double.NegativeInfinity, System.Math.Max, Min); }
    ...
    /// <param name="node">The node.</param>
    /// <returns>Node.</returns>
    public virtual Node Min(Node node) { return Find(node, double.PositiveInfinity, System.Math.Min, Max); }
  }
}