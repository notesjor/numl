...
namespace numl.Utils
{
  ...
  public static class StringHelpers
  {
    ...
    /// <param name="exclusion">(Optional) the exclusion.</param>
    /// <returns>A string array</returns>
    public static string[] BuildDistinctWordArray(
      ...
      return examples.Select(
        o =>
        {
          ...
          s =
            new string(
              s.ToCharArray()
               .Where(c => !char.IsSymbol(c) && !char.IsPunctuation(c) && !char.IsSeparator(c))
               .ToArray());
    ...
    public static int GetWordPosition(string item, string[] dictionary, bool checkNumber = true)
    {
      ...
      var index = Array.IndexOf(dictionary, item);
      if (index > -1)
        return index;
    ...
    public static string Sanitize(this string s, bool checkNumber = true)
    {
      ...
      item =
        new string(
          item.ToCharArray().Where(c => !char.IsSymbol(c) && !char.IsPunctuation(c) && !char.IsSeparator(c)).ToArray());
