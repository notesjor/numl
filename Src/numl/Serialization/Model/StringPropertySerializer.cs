...
namespace numl.Serialization.Model
{
  ...
  {
    public override bool CanConvert(Type type) { return typeof(StringProperty).IsAssignableFrom(type); }

    public override object Create() { return new StringProperty(); }

    public override object Read(JsonReader reader)
    {
      var p = (StringProperty) base.Read(reader);
      p.Separator = reader.ReadProperty().Value.ToString();
      p.SplitType = (StringSplitType) Enum.Parse(
        typeof(StringSplitType),
        reader.ReadProperty().Value.ToString());
      p.Dictionary = reader.ReadArrayProperty().Value
                           .Select(o => (string) o)
                           .ToArray();
