namespace numl.Math.Functions
{
  ...
  public class ClippedRectifiedLinear : Function
  {
    ...
    /// </summary>
    public ClippedRectifiedLinear() { MaxValue = 5d; }
    ...
    public override double Compute(double x) { return x > 0d ? (x > MaxValue ? MaxValue : x) : 0d; }

    /// <summary>Derivatives the given x coordinate.</summary>
    /// <param name="x">The Vector to process.</param>
    /// <returns>A Vector.</returns>
    public override double Derivative(double x) { return x > 0d && x < MaxValue ? 1 : 0; }
  }
}