...
namespace numl.Math.Information
{
  ...
  public abstract class Impurity
  {
    ...
    public double Conditional(Vector y, Vector x)
    {
      ...
      result += (from i in values select x.Indices(d => d == i) into s let p = s.Count() / count let h = Calculate(y.Slice(s)) select p * h).Sum();
    ...
    public double SegmentedConditional(Vector y, Vector x, IEnumerable<Range> ranges)
    {
      ...
      // aggregate results
      result += (from range in Segments select x.Indices(d => d >= range.Min && d < range.Max) into s let p = s.Count() / count let h = Calculate(y.Slice(s)) select p * h).Sum();
